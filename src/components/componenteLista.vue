...
export default {
  ...
  created() {
    ...
    if(this.$route.params.localidad){
      ...
      axios.get("https://f579jb.deta.dev/appVivienda/viviendas/"+parametro).then(response => this.viviendas = response.data);
    }else{
      axios.get("https://f579jb.deta.dev/appVivienda/viviendas").then(response => this.viviendas = response.data);
    }
  },
  beforeUpdate() { 
        // Simple GET request using axios
